"""
Experimento 10: Datasets Seeds y Glass.

Seeds: 7 features, 3 clases, 210 muestras.
  Subconfiguraciones: (2^7-1)×(2^3-1) = 889
Glass: 9 features, 6 clases, 214 muestras.
  Subconfiguraciones: (2^9-1)×(2^6-1) = 32193

Ejecución:
    julia --project=. benchmarks/exp10_seeds_glass.jl
"""

include("utils.jl")

function cargar_seeds()
    ruta = joinpath(@__DIR__, "seeds.csv")
    lines = filter(!isempty, readlines(ruta))
    n = length(lines)
    entradas = zeros(Float64, n, 7)
    clases = zeros(Int, n)

    for (i, line) in enumerate(lines)
        vals = split(line)
        for j in 1:7
            entradas[i, j] = parse(Float64, vals[j])
        end
        clases[i] = parse(Int, vals[8])
    end

    # Normalizar
    for j in 1:7
        col = entradas[:, j]
        mn, mx = minimum(col), maximum(col)
        rango = mx - mn
        rango > 0 && (entradas[:, j] .= (col .- mn) ./ rango)
    end

    # One-hot (clases 1,2,3)
    salidas = zeros(Float64, n, 3)
    for i in 1:n
        salidas[i, clases[i]] = 1.0
    end

    return entradas, salidas
end

function cargar_glass()
    ruta = joinpath(@__DIR__, "glass.csv")
    lines = filter(!isempty, readlines(ruta))
    n = length(lines)
    entradas = zeros(Float64, n, 9)
    clases_raw = zeros(Int, n)

    for (i, line) in enumerate(lines)
        vals = split(line, ',')
        # Columna 1: ID (ignorar), 2-10: features, 11: clase
        for j in 1:9
            entradas[i, j] = parse(Float64, vals[j + 1])
        end
        clases_raw[i] = parse(Int, vals[11])
    end

    # Normalizar
    for j in 1:9
        col = entradas[:, j]
        mn, mx = minimum(col), maximum(col)
        rango = mx - mn
        rango > 0 && (entradas[:, j] .= (col .- mn) ./ rango)
    end

    # Remapear clases (1,2,3,5,6,7) → (1,2,3,4,5,6)
    mapa_clases = Dict(1=>1, 2=>2, 3=>3, 5=>4, 6=>5, 7=>6)
    n_clases = 6
    salidas = zeros(Float64, n, n_clases)
    for i in 1:n
        salidas[i, mapa_clases[clases_raw[i]]] = 1.0
    end

    return entradas, salidas
end

function split_estratificado(entradas, salidas; ratio=0.8, seed=42)
    Random.seed!(seed)
    n = size(entradas, 1)
    n_clases = size(salidas, 2)
    idx_train = Int[]
    idx_test = Int[]
    for c in 1:n_clases
        idx_clase = findall(salidas[:, c] .== 1.0)
        isempty(idx_clase) && continue
        perm = randperm(length(idx_clase))
        n_train = max(1, round(Int, length(idx_clase) * ratio))
        append!(idx_train, idx_clase[perm[1:n_train]])
        if n_train < length(idx_clase)
            append!(idx_test, idx_clase[perm[n_train+1:end]])
        end
    end
    shuffle!(idx_train)
    shuffle!(idx_test)
    return (entradas=entradas[idx_train, :], salidas=salidas[idx_train, :]),
           (entradas=entradas[idx_test, :], salidas=salidas[idx_test, :])
end

function ejecutar_dataset(nombre, entradas, salidas, capas_list, seeds;
                          umbral=0.4, epochs=1000, lr=0.01, nombres_feat=nothing)
    n_ent = size(entradas, 2)
    n_sal = size(salidas, 2)
    n_muestras = size(entradas, 1)
    total_sub = (2^n_ent - 1) * (2^n_sal - 1)

    println("\n═══ $nombre ═══")
    println("$n_ent features, $n_sal clases, $n_muestras muestras")
    println("Subconfiguraciones: $total_sub | Seeds: $(length(seeds))\n")

    for capas in capas_list
        n_total = sum(capas)
        n_ocultas = capas[2]

        println("─── Red $(join(capas, "→")) ($n_total neuronas) | umbral=$umbral ───\n")

        println("┌────────┬──────────────────────┬──────────────────────┬──────────┐")
        println("│  Seed  │  Ref. Completa       │  Mejor Seleccionada  │ Tipo     │")
        println("│        │ Neur │ Post (test)    │ Neur │ Post (test)   │          │")
        println("├────────┼──────┼────────────────┼──────┼───────────────┼──────────┤")

        ref_posts = Float64[]
        sel_posts = Float64[]
        sel_neurs = Int[]
        sel_tipos = Symbol[]
        sel_ents_list = Vector{Int}[]
        tiempos = Float64[]

        for seed in seeds
            datos_train, datos_test = split_estratificado(entradas, salidas; seed=seed)
            red = crear_red(capas; seed=seed)
            config = ConfiguracionDivision{Float64}(umbral)

            t = @elapsed mapa = ejecutar_division(red, datos_test, config;
                datos_entrenamiento=datos_train,
                epochs=epochs, lr=Float64(lr), paciencia=100)

            sel = seleccionar_mejor(mapa)
            ref = mapa.referencia_completa

            push!(ref_posts, ref.precision_post_entrenamiento)
            push!(sel_posts, sel.precision)
            push!(sel_neurs, sel.neuronas)
            push!(sel_tipos, sel.tipo)
            push!(tiempos, t)

            if sel.entrada.subconfiguracion !== nothing
                push!(sel_ents_list, sel.entrada.subconfiguracion.indices_entrada)
            else
                push!(sel_ents_list, Int[])
            end

            @printf("│ %6d │ %4d │    %5.1f%%       │ %4d │    %5.1f%%      │ %-8s │\n",
                seed, n_total, ref.precision_post_entrenamiento * 100,
                sel.neuronas, sel.precision * 100, sel.tipo)
        end

        println("├────────┼──────┼────────────────┼──────┼───────────────┼──────────┤")
        n_ref = count(t -> t == :referencia, sel_tipos)
        @printf("│  Media │ %4d │    %5.1f%%       │ %4.0f │    %5.1f%%      │ ref:%-4d │\n",
            n_total, mean(ref_posts)*100, mean(sel_neurs), mean(sel_posts)*100, n_ref)
        @printf("│  Std   │      │    %5.1f%%       │ %4.1f │    %5.1f%%      │ sub:%-4d │\n",
            std(ref_posts)*100, std(sel_neurs), std(sel_posts)*100, length(seeds)-n_ref)
        println("└────────┴──────┴────────────────┴──────┴───────────────┴──────────┘")
        @printf("  Tiempo medio: %.2fs\n", mean(tiempos))

        # Feature importance
        if nombres_feat !== nothing
            conteo = zeros(Int, n_ent)
            n_sub = 0
            for ents in sel_ents_list
                length(ents) == n_ent && continue
                isempty(ents) && continue
                for idx in ents; conteo[idx] += 1; end
                n_sub += 1
            end
            if n_sub > 0
                println("  Features en subredes ($n_sub):")
                orden = sortperm(conteo, rev=true)
                for idx in orden
                    conteo[idx] == 0 && continue
                    @printf("    %2d. %-16s: %d/%d (%.0f%%)\n",
                        idx, nombres_feat[idx], conteo[idx], n_sub, conteo[idx]/n_sub*100)
                end
            end
        end
        println()
    end
end

function exp10()
    println("╔══════════════════════════════════════════════════════════════╗")
    println("║  Experimento 10: Seeds y Glass                             ║")
    println("╚══════════════════════════════════════════════════════════════╝")

    seeds = SEEDS_20

    # Seeds
    ent_s, sal_s = cargar_seeds()
    nombres_seeds = ["Area", "Perimeter", "Compactness", "KernelLen",
                     "KernelWid", "Asymmetry", "GrooveLen"]
    ejecutar_dataset("Seeds", ent_s, sal_s,
        [[7, 16, 3], [7, 32, 3]], seeds;
        nombres_feat=nombres_seeds)

    # Glass
    ent_g, sal_g = cargar_glass()
    nombres_glass = ["RI", "Na", "Mg", "Al", "Si", "K", "Ca", "Ba", "Fe"]
    ejecutar_dataset("Glass", ent_g, sal_g,
        [[9, 16, 6], [9, 32, 6]], seeds;
        nombres_feat=nombres_glass)
end

exp10()
