"""
Experimento 14: Adult Census Income.

Variante A: 6 features continuas, 2 clases, ~32K train / ~16K test.
  Features: age, fnlwgt, education-num, capital-gain, capital-loss, hours-per-week
  Subconfiguraciones: (2^6-1)×(2^2-1) = 189

Variante B: 14 features (6 continuas + 8 categóricas codificadas como ordinal), 2 clases.
  Subconfiguraciones: (2^14-1)×(2^2-1) = 49149

Ejecución:
    julia --project=. benchmarks/exp14_adult.jl
"""

include("utils.jl")

function cargar_adult(; solo_continuas::Bool=false)
    ruta_train = joinpath(@__DIR__, "..", "csv", "adult.csv")
    ruta_test = joinpath(@__DIR__, "..", "csv", "adult_test.csv")

    function parsear_lineas(lines)
        # Columnas: age, workclass, fnlwgt, education, education-num, marital-status,
        #           occupation, relationship, race, sex, capital-gain, capital-loss,
        #           hours-per-week, native-country, income
        # Continuas: 0(age), 2(fnlwgt), 4(education-num), 10(capital-gain), 11(capital-loss), 12(hours-per-week)
        # Categóricas: 1(workclass), 3(education), 5(marital), 6(occupation), 7(relationship), 8(race), 9(sex), 13(country)

        datos = []
        for line in lines
            vals = [strip(v) for v in split(line, ',')]
            length(vals) < 15 && continue
            any(v -> v == "?", strip.(vals)) && continue

            clase = occursin(">50K", vals[15]) ? 2 : 1

            if solo_continuas
                feats = Float64[
                    parse(Float64, vals[1]),   # age
                    parse(Float64, vals[3]),   # fnlwgt
                    parse(Float64, vals[5]),   # education-num
                    parse(Float64, vals[11]),  # capital-gain
                    parse(Float64, vals[12]),  # capital-loss
                    parse(Float64, vals[13])   # hours-per-week
                ]
            else
                # Codificación ordinal simple para categóricas
                workclass_map = Dict("Private"=>1, "Self-emp-not-inc"=>2, "Self-emp-inc"=>3,
                    "Federal-gov"=>4, "Local-gov"=>5, "State-gov"=>6, "Without-pay"=>7, "Never-worked"=>8)
                marital_map = Dict("Married-civ-spouse"=>1, "Divorced"=>2, "Never-married"=>3,
                    "Separated"=>4, "Widowed"=>5, "Married-spouse-absent"=>6, "Married-AF-spouse"=>7)
                relationship_map = Dict("Wife"=>1, "Own-child"=>2, "Husband"=>3,
                    "Not-in-family"=>4, "Other-relative"=>5, "Unmarried"=>6)
                race_map = Dict("White"=>1, "Asian-Pac-Islander"=>2, "Amer-Indian-Eskimo"=>3,
                    "Other"=>4, "Black"=>5)
                sex_map = Dict("Female"=>1, "Male"=>2)

                feats = Float64[
                    parse(Float64, vals[1]),                          # 1. age
                    get(workclass_map, vals[2], 0),                   # 2. workclass
                    parse(Float64, vals[3]),                          # 3. fnlwgt
                    parse(Float64, vals[5]),                          # 4. education-num
                    get(marital_map, vals[6], 0),                     # 5. marital-status
                    get(relationship_map, vals[8], 0),                # 6. relationship
                    get(race_map, vals[9], 0),                        # 7. race
                    get(sex_map, vals[10], 0),                        # 8. sex
                    parse(Float64, vals[11]),                         # 9. capital-gain
                    parse(Float64, vals[12]),                         # 10. capital-loss
                    parse(Float64, vals[13]),                         # 11. hours-per-week
                ]
            end

            push!(datos, (feats, clase))
        end
        return datos
    end

    lines_train = filter(l -> !isempty(strip(l)) && !startswith(l, "|"), readlines(ruta_train))
    lines_test = filter(l -> !isempty(strip(l)) && !startswith(l, "|"), readlines(ruta_test))

    datos_train = parsear_lineas(lines_train)
    datos_test = parsear_lineas(lines_test)

    n_feat = length(datos_train[1][1])
    n_train = length(datos_train)
    n_test = length(datos_test)

    ent_train = zeros(Float64, n_train, n_feat)
    sal_train = zeros(Float64, n_train, 2)
    for (i, (feats, clase)) in enumerate(datos_train)
        ent_train[i, :] .= feats
        sal_train[i, clase] = 1.0
    end

    ent_test = zeros(Float64, n_test, n_feat)
    sal_test = zeros(Float64, n_test, 2)
    for (i, (feats, clase)) in enumerate(datos_test)
        ent_test[i, :] .= feats
        sal_test[i, clase] = 1.0
    end

    # Normalizar usando estadísticas del train
    for j in 1:n_feat
        mn = minimum(ent_train[:, j])
        mx = maximum(ent_train[:, j])
        rango = mx - mn
        if rango > 0
            ent_train[:, j] .= (ent_train[:, j] .- mn) ./ rango
            ent_test[:, j] .= clamp.((ent_test[:, j] .- mn) ./ rango, 0.0, 1.0)
        end
    end

    return (entradas=ent_train, salidas=sal_train), (entradas=ent_test, salidas=sal_test)
end

function exp14_adult()
    println("╔══════════════════════════════════════════════════════════════╗")
    println("║  Experimento 14: Adult Census Income                       ║")
    println("╚══════════════════════════════════════════════════════════════╝")

    seeds = SEEDS_20

    # --- Variante A: 6 features continuas ---
    println("\n═══ Variante A: 6 features continuas ═══")
    datos_train_a, datos_test_a = cargar_adult(solo_continuas=true)
    n_feat = size(datos_train_a.entradas, 2)
    total_sub = (2^n_feat - 1) * (2^2 - 1)
    println("$(size(datos_train_a.entradas, 1)) train, $(size(datos_test_a.entradas, 1)) test")
    println("Subconfiguraciones: $total_sub\n")

    nombres_cont = ["Age", "FnlWgt", "EduNum", "CapGain", "CapLoss", "HoursWk"]

    for n_ocultas in [16, 32]
        capas = [n_feat, n_ocultas, 2]
        n_total = sum(capas)

        println("─── Red $(join(capas, "→")) ($n_total neur) | umbral=0.5 ───\n")

        println("┌────────┬──────────────────────┬──────────────────────┬──────────┐")
        println("│  Seed  │  Ref. Completa       │  Mejor Seleccionada  │ Tipo     │")
        println("│        │ Neur │ Post (test)    │ Neur │ Post (test)   │          │")
        println("├────────┼──────┼────────────────┼──────┼───────────────┼──────────┤")

        ref_posts = Float64[]
        sel_posts = Float64[]
        sel_neurs = Int[]
        sel_tipos = Symbol[]
        sel_ents_list = Vector{Int}[]
        tiempos = Float64[]

        for seed in seeds
            red = crear_red(capas; seed=seed)
            config = ConfiguracionDivision{Float64}(0.5)

            t = @elapsed mapa = ejecutar_division(red, datos_test_a, config;
                datos_entrenamiento=datos_train_a,
                epochs=500, lr=Float64(0.01), paciencia=50, batch_size=256)

            sel = seleccionar_mejor(mapa)
            ref = mapa.referencia_completa

            push!(ref_posts, ref.precision_post_entrenamiento)
            push!(sel_posts, sel.precision)
            push!(sel_neurs, sel.neuronas)
            push!(sel_tipos, sel.tipo)
            push!(tiempos, t)
            if sel.entrada.subconfiguracion !== nothing
                push!(sel_ents_list, sel.entrada.subconfiguracion.indices_entrada)
            else
                push!(sel_ents_list, Int[])
            end

            @printf("│ %6d │ %4d │    %5.1f%%       │ %4d │    %5.1f%%      │ %-8s │\n",
                seed, n_total, ref.precision_post_entrenamiento * 100,
                sel.neuronas, sel.precision * 100, sel.tipo)
        end

        println("├────────┼──────┼────────────────┼──────┼───────────────┼──────────┤")
        n_ref = count(t -> t == :referencia, sel_tipos)
        @printf("│  Media │ %4d │    %5.1f%%       │ %4.0f │    %5.1f%%      │ ref:%-4d │\n",
            n_total, mean(ref_posts)*100, mean(sel_neurs), mean(sel_posts)*100, n_ref)
        @printf("│  Std   │      │    %5.1f%%       │ %4.1f │    %5.1f%%      │ sub:%-4d │\n",
            std(ref_posts)*100, std(sel_neurs), std(sel_posts)*100, length(seeds)-n_ref)
        println("└────────┴──────┴────────────────┴──────┴───────────────┴──────────┘")
        @printf("  Tiempo medio: %.1fs\n", mean(tiempos))

        conteo = zeros(Int, n_feat)
        n_sub = 0
        for ents in sel_ents_list
            length(ents) == n_feat && continue
            isempty(ents) && continue
            for idx in ents; conteo[idx] += 1; end
            n_sub += 1
        end
        if n_sub > 0
            println("  Features en subredes ($n_sub):")
            orden = sortperm(conteo, rev=true)
            for idx in orden
                conteo[idx] == 0 && continue
                @printf("    %2d. %-12s: %d/%d (%.0f%%)\n",
                    idx, nombres_cont[idx], conteo[idx], n_sub, conteo[idx]/n_sub*100)
            end
        end
        println()
    end

    # --- Variante B: 11 features (continuas + categóricas codificadas) ---
    println("\n═══ Variante B: 11 features (continuas + categóricas) ═══")
    datos_train_b, datos_test_b = cargar_adult(solo_continuas=false)
    n_feat_b = size(datos_train_b.entradas, 2)
    total_sub_b = (2^n_feat_b - 1) * (2^2 - 1)
    println("$(size(datos_train_b.entradas, 1)) train, $(size(datos_test_b.entradas, 1)) test")
    println("Subconfiguraciones: $total_sub_b\n")

    nombres_all = ["Age", "Workclass", "FnlWgt", "EduNum", "Marital",
                    "Relationship", "Race", "Sex", "CapGain", "CapLoss", "HoursWk"]

    for n_ocultas in [32]
        capas = [n_feat_b, n_ocultas, 2]
        n_total = sum(capas)

        println("─── Red $(join(capas, "→")) ($n_total neur) | umbral=0.5 ───\n")

        println("┌────────┬──────────────────────┬──────────────────────┬──────────┐")
        println("│  Seed  │  Ref. Completa       │  Mejor Seleccionada  │ Tipo     │")
        println("│        │ Neur │ Post (test)    │ Neur │ Post (test)   │          │")
        println("├────────┼──────┼────────────────┼──────┼───────────────┼──────────┤")

        ref_posts = Float64[]
        sel_posts = Float64[]
        sel_neurs = Int[]
        sel_tipos = Symbol[]
        sel_ents_list = Vector{Int}[]
        tiempos = Float64[]

        for seed in seeds
            red = crear_red(capas; seed=seed)
            config = ConfiguracionDivision{Float64}(0.5)

            t = @elapsed mapa = ejecutar_division(red, datos_test_b, config;
                datos_entrenamiento=datos_train_b,
                epochs=500, lr=Float64(0.01), paciencia=50, batch_size=256)

            sel = seleccionar_mejor(mapa)
            ref = mapa.referencia_completa

            push!(ref_posts, ref.precision_post_entrenamiento)
            push!(sel_posts, sel.precision)
            push!(sel_neurs, sel.neuronas)
            push!(sel_tipos, sel.tipo)
            push!(tiempos, t)
            if sel.entrada.subconfiguracion !== nothing
                push!(sel_ents_list, sel.entrada.subconfiguracion.indices_entrada)
            else
                push!(sel_ents_list, Int[])
            end

            @printf("│ %6d │ %4d │    %5.1f%%       │ %4d │    %5.1f%%      │ %-8s │\n",
                seed, n_total, ref.precision_post_entrenamiento * 100,
                sel.neuronas, sel.precision * 100, sel.tipo)
        end

        println("├────────┼──────┼────────────────┼──────┼───────────────┼──────────┤")
        n_ref = count(t -> t == :referencia, sel_tipos)
        @printf("│  Media │ %4d │    %5.1f%%       │ %4.0f │    %5.1f%%      │ ref:%-4d │\n",
            n_total, mean(ref_posts)*100, mean(sel_neurs), mean(sel_posts)*100, n_ref)
        @printf("│  Std   │      │    %5.1f%%       │ %4.1f │    %5.1f%%      │ sub:%-4d │\n",
            std(ref_posts)*100, std(sel_neurs), std(sel_posts)*100, length(seeds)-n_ref)
        println("└────────┴──────┴────────────────┴──────┴───────────────┴──────────┘")
        @printf("  Tiempo medio: %.1fs\n", mean(tiempos))

        conteo = zeros(Int, n_feat_b)
        n_sub = 0
        for ents in sel_ents_list
            length(ents) == n_feat_b && continue
            isempty(ents) && continue
            for idx in ents; conteo[idx] += 1; end
            n_sub += 1
        end
        if n_sub > 0
            println("  Features en subredes ($n_sub):")
            orden = sortperm(conteo, rev=true)
            for idx in orden
                conteo[idx] == 0 && continue
                @printf("    %2d. %-14s: %d/%d (%.0f%%)\n",
                    idx, nombres_all[idx], conteo[idx], n_sub, conteo[idx]/n_sub*100)
            end
        end
        println()
    end
end

exp14_adult()
